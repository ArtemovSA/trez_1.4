...
void check_bootloader(void)
{
	...
	for (int i = FLASH_META_SECTOR_FIRST; i <= FLASH_META_SECTOR_LAST; i++) {
		layoutProgress("ERASE ... Please wait meta", 1000 * (i - FLASH_META_SECTOR_FIRST) / (FLASH_META_SECTOR_LAST - FLASH_META_SECTOR_FIRST));
        	flash_erase_sector(i, FLASH_CR_PROGRAM_X32);
	}
	
	for (int i = FLASH_BOOT_SECTOR_FIRST; i <= FLASH_BOOT_SECTOR_LAST; i++) {
		layoutProgress("ERASE ... Please wait boot", 1000 * (i - FLASH_BOOT_SECTOR_FIRST) / (FLASH_BOOT_SECTOR_LAST - FLASH_BOOT_SECTOR_FIRST));
                flash_erase_sector(i, FLASH_CR_PROGRAM_X32);
	}
